...
public:
    ...
    bool intersect(const Ray &ray, glm::vec3 &point, glm::vec3 &normal) {
        return (glm::intersectRaySphere(ray.p, ray.d, position, lightradius, point, normal));
    }
    Light(){
    }
    float intensity = 50.0;
    void draw(){
        ofDrawSphere(position, lightradius);
    }
...
class ofApp : public ofBaseApp{
    ...
    public:
        ...
        ofColor shade(const glm::vec3 &p, const glm::vec3 &norm, const ofColor diffuse, const ofColor specular, float power);
        ofColor antiAlias(const float uCenter, const float vCenter, const float widthPixel, const float heightPixel);
        bool myRayIsBlocked(Ray ray);
        ...
        glm::vec3 closestIntersectPt;
        glm::vec3 closestIntersectNorm;
    
        //Gui variables
        ofxFloatSlider lightIntensity;
        ofxFloatSlider powerExp;
        ofxFloatSlider sphereSize;
        ofxIntSlider oversamplingNum;
